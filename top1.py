...
tf.flags.DEFINE_string('logs_dir', 'logs_triplet/', 'path to logs directory')
...
def main(argv=None):
    ...
    network_factory = net.create_network_factory(
            is_training=False, num_classes=1500 + 1,
            add_logits= None)
        ...
        if FLAGS.mode == 'train':
          ...
          print cmc_total
            ...
            for times in xrange(do_times):  
                ...
                q_feat = sess.run(feature_var, feed_dict=feed_dict)
                
                feed_dict = {images_total: test_feature, is_train: False}
                test_feat = sess.run(feature_var, feed_dict=feed_dict)
                ...
                pick = tf_q_feat[h]
                tf_q_feat = tf.reshape(pick,[1,128])
                
                
                #feat1 = tf.tile(tf_q_feat,[100,1])
                
                d = cosine_distance(tf_q_feat,tf_test_feat) #100,100
                d = tf.reshape(d,[100])
